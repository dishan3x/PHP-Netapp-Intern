 ...
 *	Page consist of content of the languague preferences accoording to the user.      * 
 *	Users are devided according to the language	                                     *
 *                                                                                   *
 ************************************************************************************/
...
#information_display{ width : 500px; }

.glyphicon-minus: deletepopup  {
...
<div id = 'selection_display'>
		...
		<div id = 'th' class = 'heading sidebar_subheader'><span>Language Preferences<span></div>
					
	
		<div id = 'mh' class = 'heading'>
			<ul class='nav nav-tabs nav-justified' id = 'tabrow' role='tablist'>
				<li role='presentation' id = 'languageType' class='active'><a href='#' aria-controls='languages' role='tab' data-toggle='tab'>Languages</a></li>
			</ul>
		</div>
	...
	<!-- language types -->
		...
		<div class="deletepopup"><span>Delete</span></div>
...
<div id = 'information_display'>
	...
	<div id="languague" class='heading sidebar_subheader'>
		<span id="languague_subheader" class='sidebar_subheader_label'></span>
		<i id="manage_languagues_back_button" class="material-icons close_btn" >navigate_before</i>
	</div>
		
		<div id = 'mh' class = 'heading'>
			<ul class='nav nav-tabs nav-justified' id = 'tabrow' role='tablist'>
				<li role='presentation' id = 'languageTypeUsers' class='active'><a href='#' aria-controls='languages' role='tab' data-toggle='tab'>People</a></li>
			</ul>
		</div>
		...
		<div id = 'disp_info_div' style='width : 100%' >
...
$(document).ready(function() {
		...
		$("#languageData").on('click','.languageBtnClass',function(e){
			...
			$('#languageTypeUsers').html(" <a href='#' aria-controls='languages' role='tab' data-toggle='tab'>"+button_text + "</a>");
			...
			console.log("funcname: " + funcname + " value: " + button_id);
		 	$("#disp_info_div").load("API/search/search_users_by_language.php?funcname=" +encodeURIComponent(funcname) + "&val=" +encodeURIComponent(button_id) + "&val2=" +encodeURIComponent(button_id) + "&txt=" + encodeURIComponent($(this).text()), function(response, status, xhr) {	
				if(status == "error") {
					var msg = "Error: ";
		...
		// change the color to yellow when the mouse moves over to delete button
			...
			if($("#languageAdd").val().length> 0 && $("#languageCodeAdd").val().length> 0  ){
				if (confirm("Are you sure you want to do this action?")){	
					...
					$("#disp_info_div").load("search_users_by_language.php?funcname=" +encodeURIComponent(funcname) + "&val=" +encodeURIComponent(Addlang) + "&val2=" +encodeURIComponent(AddlangCode) +"&txt=" + encodeURIComponent($(this).text()), function(response, status, xhr) {	
						...
						$("#add_div").hide();
						$("#languageData").load("../API/views/languages.php"); // reload the modified content	(add contennt)	
						toastDat("success", "language successfully added");
					});	
				} 
			}
		...
		$("#manage_languagues_back_button").click(function(e){
		...
		$("#languageData").on('click','.delete_button',function(e){
			...
			if (confirm("Are you sure you want delete this language?")){
				...
				$("#disp_info_div").load("API/search/search_users_by_language.php?funcname=" +encodeURIComponent(funcname) + "&val=" +encodeURIComponent(deletelang) + "&val2=" +encodeURIComponent(deletelang) + "&txt=" + encodeURIComponent($(this).text()), function(response, status, xhr) {
					if(status == "error") {
						var msg = "Error: ";
						console.log(msg + xhr.status + " " + xhr.statusText);
					}
						...
						console.log("deleted");
						$("#languageData").load("../API/views/languages.php"); // reload the modified content
						toastDat("success", "language successfully removed");
					}
				});	
			}	
			$("#selection_display").toggle();
		...
		$('.proficiencyLevel').on('click',function(){
					
			var idval = $(this).parent().data('btn');
			var username =$(this).parent().data('username');	
			var prof    = $(this).parent().data('search');	
			console.log(idval);
